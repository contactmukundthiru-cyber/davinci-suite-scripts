...
def _save_report(report: Report, output_dir: Path) -> None:
    ...
    base = f"t6_timeline_normalizer_{now_stamp()}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="t6_timeline_normalizer")
    ...
    ensure_dirs(cfg)
    setup_logging(cfg)
    logger = get_logger("standalone", tool_id="t6_timeline_normalizer")
    ...
    tool = TimelineNormalizer(ctx)
    try:
        report = tool.run(options)
    except ResolveConnectionError as exc:
        report = Report(tool_id="t6_timeline_normalizer", title="t6_timeline_normalizer")
