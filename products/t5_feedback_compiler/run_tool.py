...
def _save_report(report: Report, output_dir: Path) -> None:
    ...
    base = f"t5_feedback_compiler_{now_stamp()}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="t5_feedback_compiler")
    ...
    ensure_dirs(cfg)
    setup_logging(cfg)
    logger = get_logger("standalone", tool_id="t5_feedback_compiler")
    ...
    tool = FeedbackCompiler(ctx)
    try:
        report = tool.run(options)
    except ResolveConnectionError as exc:
        report = Report(tool_id="t5_feedback_compiler", title="t5_feedback_compiler")
