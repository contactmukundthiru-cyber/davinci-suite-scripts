...
def _save_report(report: Report, output_dir: Path) -> None:
    ...
    base = f"t9_change_impact_analyzer_{now_stamp()}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="t9_change_impact_analyzer")
    ...
    ensure_dirs(cfg)
    setup_logging(cfg)
    logger = get_logger("standalone", tool_id="t9_change_impact_analyzer")
    ...
    tool = ChangeImpactAnalyzer(ctx)
    try:
        report = tool.run(options)
    except ResolveConnectionError as exc:
        report = Report(tool_id="t9_change_impact_analyzer", title="t9_change_impact_analyzer")
