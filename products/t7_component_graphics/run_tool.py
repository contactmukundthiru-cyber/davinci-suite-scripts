...
def _save_report(report: Report, output_dir: Path) -> None:
    ...
    base = f"t7_component_graphics_{now_stamp()}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="t7_component_graphics")
    ...
    ensure_dirs(cfg)
    setup_logging(cfg)
    logger = get_logger("standalone", tool_id="t7_component_graphics")
    ...
    tool = ComponentGraphicsSystem(ctx)
    try:
        report = tool.run(options)
    except ResolveConnectionError as exc:
        report = Report(tool_id="t7_component_graphics", title="t7_component_graphics")
