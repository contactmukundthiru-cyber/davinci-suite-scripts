...
def _save_report(report: Report, output_dir: Path) -> None:
    ...
    base = f"t2_relink_across_projects_{now_stamp()}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="t2_relink_across_projects")
    ...
    ensure_dirs(cfg)
    setup_logging(cfg)
    logger = get_logger("standalone", tool_id="t2_relink_across_projects")
    ...
    tool = RelinkAcrossProjects(ctx)
    try:
        report = tool.run(options)
    except ResolveConnectionError as exc:
        report = Report(tool_id="t2_relink_across_projects", title="t2_relink_across_projects")
