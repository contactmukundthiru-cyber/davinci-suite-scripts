...
def _save_report(report: Report, output_dir: Path) -> None:
    ...
    base = f"t1_revision_resolver_{now_stamp()}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="t1_revision_resolver")
    ...
    ensure_dirs(cfg)
    setup_logging(cfg)
    logger = get_logger("standalone", tool_id="t1_revision_resolver")
    ...
    tool = RevisionResolver(ctx)
    try:
        report = tool.run(options)
    except ResolveConnectionError as exc:
        report = Report(tool_id="t1_revision_resolver", title="t1_revision_resolver")
