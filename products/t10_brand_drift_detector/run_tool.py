...
def _save_report(report: Report, output_dir: Path) -> None:
    ...
    base = f"t10_brand_drift_detector_{now_stamp()}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="t10_brand_drift_detector")
    ...
    ensure_dirs(cfg)
    setup_logging(cfg)
    logger = get_logger("standalone", tool_id="t10_brand_drift_detector")
    ...
    tool = BrandDriftDetector(ctx)
    try:
        report = tool.run(options)
    except ResolveConnectionError as exc:
        report = Report(tool_id="t10_brand_drift_detector", title="t10_brand_drift_detector")
