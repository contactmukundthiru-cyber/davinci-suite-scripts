...
def _save_report(report: Report, output_dir: Path) -> None:
    ...
    base = f"t4_caption_layout_protector_{now_stamp()}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="t4_caption_layout_protector")
    ...
    ensure_dirs(cfg)
    setup_logging(cfg)
    logger = get_logger("standalone", tool_id="t4_caption_layout_protector")
    ...
    tool = CaptionLayoutProtector(ctx)
    try:
        report = tool.run(options)
    except ResolveConnectionError as exc:
        report = Report(tool_id="t4_caption_layout_protector", title="t4_caption_layout_protector")
